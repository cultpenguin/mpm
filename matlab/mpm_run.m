% _run : run MPM binary from within matlab

function [geou,geow,options]=mpm_run(x,z,vp,vs,rho,dt,mf,xs,zs,options)
options.null=[];
geou=[];
geow=[];
...
if exist('mpm_bin')~=2
    mpm_bin=[fileparts(which(mfilename)),filesep,'..',filesep,'src',filesep,'mpm'];
    if ~isunix
        mpm_bin=[mpm_bin,'.exe'];
    end
end
